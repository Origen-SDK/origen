...
# If you want to add a nested sub-block to this model then run the following command,
# replacing my_sub_block_name with the name of your sub-block:
#   origen new sub_block <%= Pathname.new(resource_path_to_models_dir(@resource_path)).relative_path_from(Origen.root) %> my_sub_block_name
#
...
# sub_block :nvm, load_model: 'features/memory'       # Add the definitions from the given model(s)
# sub_block :nvm, load_model: ['features/memory', 'features/testable']
