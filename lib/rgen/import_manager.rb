module RGen
  ...
  class ImportManager
    ...
    def initialize
      ...
      @required_rgen_version = RGen.config.required_rgen_version ||
                               RGen.config.min_required_rgen_version
      @max_required_rgen_version = RGen.config.max_required_rgen_version
    end
      ...
      unless required?
        ...
        if @required_rgen_version
          if @max_required_rgen_version
            if RGen::VersionString.new(@required_rgen_version).greater_than?(@max_required_rgen_version)
              puts ''
              puts "Your application needs to run RGen version #{@required_rgen_version}, however"
              puts "the version of the #{@max_lib} plugin required by your import tree is not"
              puts "permitted to run above RGen version #{@max_required_rgen_version}."
          ...
          if RGen.version.less_than?(@required_rgen_version)
            if RGen.config.required_rgen_version
              puts ''
              puts "A dependent plugin requires at least RGen version #{@required_rgen_version}, however"
              puts "your main application currently specifies #{RGen.config.required_rgen_version}."
              puts 'To proceed you must update the required_rgen_version in config/application.rb'
              puts "to be: #{@required_rgen_version}"
              puts ''
              exit 0
            else
              RGen.version_checker.update_rgen(@required_rgen_version)
            end
          end
        end
    ...
    def validate_production_status(force = false)
      if RGen.mode.production? || force
        ...
        if File.exist?("#{RGen.root}/Gemfile")
          File.readlines("#{RGen.root}/Gemfile").each do |line|
    ...
    def plugin_version(plugin_name)
      load File.join(rgen_root_for(imports[plugin_name]), 'config', 'version.rb')
      RGen::VersionString.new(RGen.import_manager.plugin_instance(plugin_name).class::VERSION)
    end

    # Returns the app instance of the given plugin name
    def plugin_instance(name)
      RGen.application_instance(rgen_root_for(imports[name]))
    end

    # Returns an array of symbols that represent the names of all imports
    def names
      return @names if @names
      names = RGen.plugins.map(&:name)
      # Had a bug where this was caching too early, don't cache until all plugins are loaded
      @names = names if RGen.app_loaded?
      names
    end
    ...
    # Returns the path to rgen root for the given import name
    def rgen_root(name)
      rgen_root_for(named_imports[name])
    end
    ...
    def delete_symlinks_of_plugin(plugin)
      SHARED_CONTENTS_TYPES.each do |type|
        unless type == :command_launcher
          link = "#{RGen.root}/#{type}/#{plugin.name}"
          if File.exist?(link)
    ...
    def require_environments!
      ...
      ordered_imports.reverse_each do |name|
        root = rgen_root_for(imports[name.to_sym])
        RGen.with_rgen_root(root) do
    ...
    def remove_unwanted_symlinks!
      SHARED_CONTENTS_TYPES.each do |type|
        symlinks = symlinks_in_dir("#{RGen.root}/#{type}") unless type == :command_launcher
        if symlinks
    ...
    def add_shared_contents!
      [RGen.app, RGen.plugins].flatten.each do |plugin|
        ...
        if shared_content
          shared_content.each do |type, data|
            ...
            if type && type != :command_launcher
              ...
              #  FileUtils.mkdir_p("#{RGen.root}/#{type}") unless File.exist?("#{RGen.root}/#{type}")
              #  unless File.exist?("#{RGen.root}/#{type}/#{plugin.name}/")
              #    create_symlink("#{plugin.root}/#{data}", "#{RGen.root}/#{type}/#{plugin.name}")
              #  end
              # else
    ...
    def rgen_root_for(import, options = {})
      ...
      if File.exist?("#{workspace}/config/application.rb")
        ...
        root = workspace.join('tool_data', 'rgen')
      end
    ...
    def updates_required?
      @current_app_name = RGen.app.name
      @current_app = {
        vault: RGen.app.config.vault,
        path:  RGen.root
      }
      resolve_imports
    ...
    def top_level_imports
      RGen.app.config.imports + RGen.app.config.imports_dev
    end
    ...
    #
    # This will also update the required rgen version if a app
    ...
    def traverse_imports(import, &block)
      ...
      if import_present?(import)
        app = RGen.application_instance(rgen_root_for(import), reload: true)
        update_required_rgen_version(app)
    ...
    # If the given app (plugin) requires a newer RGen than the current latest
    # required version then the required version parameter will be updated.
    def update_required_rgen_version(app)
      min = app.config.required_rgen_version || app.config.min_required_rgen_version
      max = app.config.max_required_rgen_version
      if min
        if @required_rgen_version
          latest = RGen::VersionString.new(@required_rgen_version)
          if latest.less_than?(min)
            @required_rgen_version = min
          end
        else
          @required_rgen_version = min
        end
      ...
      if max
        if @max_required_rgen_version
          latest = RGen::VersionString.new(@max_required_rgen_version)
          if latest.less_than?(max)
            @max_required_rgen_version = max
    ...
    def import_present?(import)
      !!rgen_root_for(import, accept_missing: true)
    end
    ...
    def add_import(new)
      ...
      if existing
        if new[:path]
          ...
          existing_version = RGen::VersionString.new(existing[:version])
