module RGen
  # RGen provides a powerful register class which you are encouraged to use when
  ...
  module Registers
    ...
    autoload :Container,     'rgen/registers/container'
    autoload :Reg,           'rgen/registers/reg'
    autoload :Bit,           'rgen/registers/bit'
    autoload :BitCollection, 'rgen/registers/bit_collection'
    autoload :RegCollection, 'rgen/registers/reg_collection'
    autoload :Domain,        'rgen/registers/domain'

    included do
      include RGen::ModelInitializer
      include RGen::SubBlocks
      include RGen::Callbacks  # Required for global register reset
    ...
    class << self
      ...
      def default_reg_metadata
        RGen::Registers.reg_metadata[:global] ||= {}
        if block_given?
          collector = Collector.new
          yield collector
          RGen::Registers.reg_metadata[:global].merge!(collector.store)
        end
        RGen::Registers.reg_metadata[:global]
      end
      ...
      def default_bit_metadata
        RGen::Registers.bit_metadata[:global] ||= {}
        if block_given?
          collector = Collector.new
          yield collector
          RGen::Registers.bit_metadata[:global].merge!(collector.store)
        end
        RGen::Registers.bit_metadata[:global]
      end
        ...
        if feature
          ...
          if feature.class == Array
            feature.each do |f|
              ...
              unless value
                if RGen.top_level && \
                   RGen.top_level.respond_to?(:has_feature?) && \
                   RGen.top_level.has_feature?(f)
                  value = true
                  unless value
                    break
                  end
            ...
            unless value
              if RGen.top_level && \
                 RGen.top_level.respond_to?(:has_feature?) && \
                 RGen.top_level.has_feature?(feature)
                value = true
              end
            end
            return value
    ...
    # Returns true if the given object is one of the recognized RGen
    # bit containers (bit collection, reg or container).
    def contains_bits?(obj)
      obj.respond_to?(:contains_bits?) && obj.contains_bits?
    end

    # Returns true if the given object is an RGen bit
    def is_a_bit?(obj)
      obj.is_a?(RGen::Registers::Bit)
    end
    ...
    def add_reg(id, address, size = nil, bit_info = {}, &_block)
      ...
      if _registers[id] && RGen.config.strict_errors
        puts ''
    ...
    def default_reg_metadata
      RGen::Registers.reg_metadata[self.class] ||= {}
      if block_given?
        collector = Collector.new
        yield collector
        RGen::Registers.reg_metadata[self.class].merge!(collector.store)
      end
      RGen::Registers.reg_metadata[self.class]
    end
    ...
    def default_bit_metadata
      RGen::Registers.bit_metadata[self.class] ||= {}
      if block_given?
        collector = Collector.new
        yield collector
        RGen::Registers.bit_metadata[self.class].merge!(collector.store)
      end
      RGen::Registers.bit_metadata[self.class]
    end
    ...
    def instantiate_reg(id, attrs)
      return _registers[id] unless _registers[id].is_a?(RGen::Registers::Placeholder)
      attributes = {
