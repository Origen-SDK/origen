module RGen
  class Application
    ...
    # to the LSF. This is in contrast to RGen::Application::LSF which is an API for
    # talking to the LSF.
    class LSFManager
      include Callbacks
      ...
      def print_status(options = {})
        ...
        RGen.log.info ''
        RGen.log.info 'LSF Status'
        RGen.log.info '----------'
        RGen.log.info "Queuing:    #{queuing_jobs.size}"
        RGen.log.info "Running:    #{running_jobs.size}"
        RGen.log.info "Lost:       #{lost_jobs.size}"
        RGen.log.info ''
        RGen.log.info "Passed:     #{passed_jobs.size}"
        RGen.log.info "Failed:     #{failed_jobs.size}"
        RGen.log.info ''
        if options[:print_insructions]
          RGen.log.info 'Common tasks'
          RGen.log.info '------------'
          if queuing_jobs.size > 0
            RGen.log.info 'Queuing'
            RGen.log.info ' Show details: rgen l -v -t queuing'
            RGen.log.info ' Re-submit:    rgen l -r -t queuing'
          end
          if running_jobs.size > 0
            RGen.log.info 'Running'
            RGen.log.info ' Show details: rgen l -v -t running'
            RGen.log.info ' Re-submit:    rgen l -r -t running'
          end
          if lost_jobs.size > 0
            RGen.log.info 'Lost'
            RGen.log.info ' Show details: rgen l -v -t lost'
            RGen.log.info ' Re-submit:    rgen l -r -t lost'
          end
          if passed_jobs.size > 0
            RGen.log.info 'Passed'
            RGen.log.info ' Build log:    rgen l -l'
          end
          if failed_jobs.size > 0
            RGen.log.info 'Failed'
            RGen.log.info ' Show details: rgen l -v -t failed'
            RGen.log.info ' Re-submit:    rgen l -r -t failed'
          end
          RGen.log.info ''
          RGen.log.info 'Reset the LSF manager (clear all jobs): rgen lsf -c -t all'
          RGen.log.info ''
        end
      end
      ...
      def print_details(options = {})
        if options[:id]
          RGen.log.info "Job: #{options[:id]}"
          RGen.log.info '----' + '-' * options[:id].length
      ...
      def print_details_of(job)
        RGen.log.info "#{job[:command]} #{job[:switches]}".gsub(' --exec_remote', '')
        RGen.log.info "ID: #{job[:id]}"
        RGen.log.info "Submitted: #{time_ago(job[:submitted_at])}"
        RGen.log.info ''
      end
              ...
              begin
                ...
                if line =~ /Total patterns:\s+(\d+)/
                  ...
                  if line =~ /^\s*$/ || line =~ /.*\|\|\s*$/
                    ...
                    # Screen std rgen output
                    unless line =~ /  rgen save/ ||
      ...
      def resubmit_job(job)
        ...
        job[:lsf_id] = RGen.app.lsf.submit(command_prefix(job[:id], job[:dependents_ids]) + job[:command] + job[:switches], dependents: job[:dependents_lsf_ids])
        job[:status] = nil
      ...
      def submit_job(command, options = {})
        ...
        lsf_id = RGen.app.lsf.submit(command_prefix(id, dependents_ids) + command + switches, dependents: dependents_lsf_ids)
        job_attrs = {
          id:                 id,
          lsf_id:             lsf_id,
          command:            command,
      ...
      def submit_rgen_job(cmd, options = {})
        if options[:action]
        ...
        str.sub('rgen ', '') if str =~ /^rgen /

        # Append the --exec_remote switch to all RGen commands, this allows command
        # processing to be altered based on whether it is running locally or
        # remotely by testing RGen.running_remotely?
        str += ' --exec_remote'

        submit_job("rgen #{str}", options)
      end

      def command_prefix(id, dependents)
        rgen = `which rgen`
        # http://rubular.com/r/wgKi73KjUo
        if rgen =~ /(^\/run\/pkg\/fs-rgen-\/[^\/]+)/
          prefix = "source #{Regexp.last_match[1]}/rgen_setup; "
        else
          prefix = "cd #{RGen.top}; source source_setup; "
        end
        prefix += "cd #{RGen.root}; rgen l --execute --id #{id} "
      ...
      def classify_jobs
        clear_caches
        queuing_job_ids = RGen.app.lsf.queuing_job_ids
        running_job_ids = RGen.app.lsf.running_job_ids
        remote_jobs.each do |_id, job|
