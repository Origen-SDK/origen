module RGen
  class Application
    class WorkspaceManager
      ...
      # root (basically just RGen.app.rc.root.parent)
      ...
      #
      # This may not necessarily be RGen.root if the application is embedded within
      # a larger project workspace (for example in tool_data/rgen)
      def revision_control_root
        RGen.app.rc.root
      end

      # RGen.root may not necessarily be the same as the revision control root.
      # This method will return the relative path from the revision control root to
      # RGen.root.
      def path_to_rgen_root
        path = RGen.root.to_s.sub(revision_control_root.to_s, '').sub(/^(\/|\\)/, '')
        path = '.' if path.empty?
        path
      end
      ...
      def reference_workspace_proposal
        "#{container_directory}/#{RGen.app.name}_reference"
      end
      ...
      def reference_workspace
        if reference_workspace_set?
          ...
          dir.gsub!('.ref', '')
          dir.gsub!(/#{Regexp.escape(path_to_rgen_root)}\/?$/, '')
      ...
      def set_reference_workspace(workspace)
        ...
        remote_ref = "#{rgen_root(workspace)}/.ref"
      ...
      # Returns the full path to RGen.root within the given workspace
      def rgen_root(workspace)
        Pathname.new("#{workspace}/#{path_to_rgen_root}").cleanpath
      end

      def reference_dir
        "#{RGen.root}/.ref" # Should probably be set by a config parameter
      end

      # Builds a new workspace at the given path
      def build(path, options = {})
        options = {
          rc_url:        RGen.app.config.rc_url || RGen.app.config.vault,
          allow_rebuild: false
        }.merge(options)
      ...
      def switch_version(workspace, tag, options = {})
        options = {
          rgen_root_only: false, # When true pop the RGen.root dir only instead
        ...
        if options[:rgen_root_only]
          dir = "#{workspace}/#{path_to_rgen_root}"
        else
          dir = workspace
        end
        rc_url = RGen.app.config.rc_url || RGen.app.config.vault
