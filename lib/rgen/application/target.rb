    ...
    class Target
      ...
      def loop(options = {})
        ...
        targets.each do |target|
          RGen.load_target(target, options) if set
      ...
      # option.
      # === Example
      #   RGen.app.target.each_production do
      ...
      # === Example
      #   RGen.app.target.each_unique_production do |moos|
      ...
      def find(name)
        if name
          ...
          if File.exist?(name)
            [name]
          elsif File.exist?("#{RGen.root}/target/#{name}") && name != ''
            ["#{RGen.root}/target/#{name}"]
          else
      ...
      #      # This attribute is used by the RGen#compile and RGen#merge tasks to allow files
      ...
      def prod_targets # :nodoc:
        return {} unless RGen.config.production_targets
        return @prod_targets if @prod_targets
        @prod_targets = {}
        RGen.config.production_targets.each do |key, value|
