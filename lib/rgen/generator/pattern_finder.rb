  ...
  class Generator
    ...
    class PatternFinder
      def find(name, options)
        ...
        if RGen.config.pattern_prefix && name =~ /^#{RGen.config.pattern_prefix}_/
          name.gsub!(/^#{RGen.config.pattern_prefix}_/, '')
        end
        ...
        if RGen.config.pattern_postfix && name =~ /_#{RGen.config.pattern_postfix}$/
          name.gsub!(/_#{RGen.config.pattern_postfix}$/, '')
        end
        ...
        if pats.size == 0
          ...
          translation = RGen.config.pattern_name_translator(name)
          if translation
      ...
      def current_plugin_pattern_path
        if RGen.current_plugin.name && RGen.current_plugin.instance.config.shared
          File.join RGen.current_plugin.instance.root,
                    RGen.current_plugin.instance.config.shared[:patterns] || RGen.current_plugin.instance.config.shared[:pattern]
        end
      end
        ...
        if file_plugin
          if RGen.current_plugin.name
            if file_plugin == RGen.current_plugin.name
              ...
              puts "The requested pattern is from plugin #{file_plugin} and current system plugin is set to plugin #{RGen.current_plugin.name}!"
      ...
      def ambiguous_error(pats)
        if RGen.running_locally?
          RGen.log.info 'The following patterns match:'
          RGen.log.info pats
        end
