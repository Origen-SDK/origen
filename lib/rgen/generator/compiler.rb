module RGen
  class Generator
    class Compiler # :nodoc: all
      ...
      #
      # @example
      #   RGen.generator.compiler.current_file   # => Pathname
      ...
      def compile(file_or_dir, options = {})
        ...
        if options[:sub_template]
          ...
          RGen.file_handler.resolve_files(file_or_dir, ignore_with_prefix: '_', import: :template) do |file|
      ...
      def merge(file_or_dir, options = {})
        ...
        diffs = []
        RGen.file_handler.resolve_files(file_or_dir, ignore_with_prefix: '_') do |file|
      ...
      def compile_file(file, options = {})
        ...
        # not be an issue except when testing RGen and generating and compiling within
        # the same thread, but clearing this here doesn't seem to do any harm.
        RGen.file_handler.default_extension = nil
        RGen.log.info "Compiling... #{relative_path_to(file)}" unless options[:quiet]
        RGen.log.info "  Created... #{relative_path_to(output_file(file, options))}" unless options[:quiet]
        ...
        if options[:zip]
          ...
          if @check_for_changes
            check_for_changes(output_file(file, options), reference_file(file, options),
                              comment_char: RGen.app.tester ? RGen.app.tester.program_comment_char : nil,
                              compile_job:  true)
          end
        end
      end
      ...
      def run_erb(file, opts = {}, &block)
        ...
        if block_given?
          ...
          content = ERB.new(File.read(file.to_s), 0, RGen.config.erb_trim_mode, buffer_name_for(file)).result(b)
        end
        insert(content)
      end
      ...
      def merge_file(file, _options = {})
        ...
        RGen.log.info "Merging... #{file.basename}"
      ...
      def check_for_differences(a, b, file)
        if check_for_changes(a, b, comment_char: ["'", 'logprint'], quiet: true, compile_job: true)
          puts "*** CHANGE DETECTED *** To rollback:  #{RGen.config.copy_command} #{display_path_to(b)} #{display_path_to(a)}"
          "#{RGen.config.diff_command} #{display_path_to(a)} #{display_path_to(b)} & #{ENV['EDITOR']} #{file.cleanpath} &"
        end
      end

      # Returns true if the supplied file name has a .erb extension
      def is_erb?(file)
        !!(file.to_s =~ /.erb$/) || !RGen.config.compile_only_dot_erb_files
      end
      ...
      def output_file(file, options = {})
        ...
        sub_dir = options[:output_sub_dir] || RGen.file_handler.sub_dir_of(file).to_s
      ...
      def reference_file(file, options = {})
        ...
        sub_dir = RGen.file_handler.sub_dir_of(file).to_s
        sub_dir = nil if sub_dir == '.'
        filename = options[:output_file_name] || file.basename.to_s.gsub('.erb', '')
        # filename.gsub!('target', $target.id) if filename =~ /target/ && $target.id
