module RGen
  module Pins
    class Pin
      ...
      #   pin.to_vector   # => "L"
      def to_vector
        @vector_formatted_value ||= RGen.tester.format_pin_state(self)
      end
      ...
      def vector_formatted_value=(val)
        unless @vector_formatted_value == val
          RGen.tester.update_pin_from_formatted_state(self, val)
      ...
      def describe(options = {})
        ...
        unless RGen.top_level.modes.empty?
          desc << ''
          desc << 'Modes'
          desc << '-------'
          RGen.top_level.modes.each do |name|
      ...
      def groups
        # RGen.pin_bank.all_pin_groups.select do |name, group|
        @groups ||= RGen.pin_bank.pin_groups.select do |_name, group|
          group.include?(self)
        end
      end
      ...
      def add_alias(id, options = {})
        ...
        if aliases[id]
          ...
          RGen.pin_bank.register_alias(id, obj, options)
        end
      end
      ...
      def is_alias_of?(name)
        if RGen.pin_bank.find(name)
          RGen.pin_bank.find(name).id == RGen.pin_bank.find(self).id
        else
          false
        end
      end
