module RGen
  module GlobalMethods
    ...
    # If a new gem (i.e. not part of the existing Ruby installation) is required by RGen or an
    ...
    def require_gem(name, options = {})
      RGen.deprecate <<-END
require_gem will be removed in RGen V3, Bundler should be used to manage gem dependencies
END
      options = {
        name: name
      }.merge(options)
      ...
      # This gem was not included in the initial RGen v2.x.x gemset, so need to handle instalations
      # without it
      begin
        if options[:version]
        ...
        if RGen.running_on_windows?
          ...
          if !system(command)
            puts 'It looks like a problem occurred, ensure you have installed Ruby exactly per the RGen guide'
          else
    ...
    def annotate(msg, options = {})
      RGen.app.tester.annotate(msg, options)
    end

    def c1(msg, options = {})
      RGen.app.tester.c1(msg, options)
    end
    alias_method :cc, :c1

    def c2(msg, options = {})
      RGen.app.tester.c2(msg, options)
    end

    def ss(*args, &block)
      RGen.app.tester.ss(*args, &block)
    end
    alias_method :step_comment, :ss

    def pp(*args, &block)
      RGen.app.tester.pattern_section(*args, &block)
    end
    alias_method :pattern_section, :pp
    alias_method :ps, :pp

    def snip(*args, &block)
      RGen.app.tester.snip(*args, &block)
    end

    # Render an ERB template
    def render(*args, &block)
      RGen.generator.compiler.render(*args, &block)
    end
    ...
    def options
      $_target_options ||
        RGen.generator.compiler.options
    end
    ...
    def debug(*lines)
      RGen.deprecate 'debug method is deprecated, use RGen.log.debug instead'
      if RGen.debug?
        RGen.log.info ''
        c = caller[0]
        c =~ /(.*):(\d+):.*/
        ...
        $_last_log_time = Time.now
        begin
          RGen.log.info "*** Debug *** %.6f #{Regexp.last_match[1]}:#{Regexp.last_match[2]}" % delta
        rescue
        ...
        lines.flatten.each do |line|
          line.split(/\n/).each do |line|
            RGen.log.info line, options
          end
        end
      end
    end

    Pattern = RGen.pattern unless defined?(Pattern)
    Flow = RGen.flow unless defined?(Flow)
    Resources = RGen.resources unless defined?(Resources)
    User = RGen::Users::User unless defined?(User)
    ...
    def encoding_search(symbol, options = {})
      ...
      results = RGen::ENCODINGS[options[:format]].filter(symbol)
      if results.size == 1
