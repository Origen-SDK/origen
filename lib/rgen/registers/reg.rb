module RGen
  module Registers
    ...
    # Any registers instantiated through RGen::Registers#add_reg are instances of this class.
    #
    # All methods in BitCollection can also be called on a Reg object.
    class Reg
      include RGen::SubBlocks::Path
      include RGen::SubBlocks::Domains
      ...
      # by RGen based on the parent's base address
      ...
      def default_reg_metadata
        RGen::Registers.default_reg_metadata.merge(
          RGen::Registers.reg_metadata[owner.class] || {})
      end
      ...
      def lookup_operation_handler(operation)
        ...
        if reg_owner.respond_to?(operation)
          ...
          reg_owner.owner
        elsif RGen.top_level && RGen.top_level.respond_to?(operation)
          RGen.top_level
        end
      end
      ...
      def address(options = {})
        ...
        if options[:address_type]
          RGen.deprecate 'Specifying the address_type of a register address will be removed from RGen 3'
      ...
      def bit(*args)
        ...
        if collection.size == 0
          # Originally RGen returned nil when asking for a bit via an index which does not
          # exist, e.g. reg[1000] => nil
          # The args numeric clause here is to maintain that behavior
          if RGen.config.strict_errors && !args.all? { |arg| arg.is_a?(Numeric) }
