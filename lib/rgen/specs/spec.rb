  ...
  module Specs
    class Spec
      ...
      Limit = Struct.new(:exp) do
        def value
          RGen::Specs::Spec.send(:evaluate_limit, exp)
        end
      end

      TYPES = RGen::Specs::SPEC_TYPES
      ...
      def diff(compare_spec)
        ...
        instance_variables.each do |ivar|
          ...
          if compare_spec.respond_to? ivar_sym
            ...
            if instance_variable_get(ivar).class == RGen::Specs::Spec::Limit
              ...
              unless instance_variable_get(ivar) == compare_spec.instance_variable_get(ivar)
                ...
                RGen.log.debug "Found spec difference for instance variable #{ivar} for #{self} and #{compare_spec}"
              end
            end
          else
            ...
            if instance_variable_get(ivar).class == RGen::Specs::Spec::Limit
              ...
              RGen.log.debug "Instance variable #{ivar} exists for #{self} and does not for #{compare_spec}"
      ...
      # RGen::Specs#specs can return a single Spec instance or an Array of Specs
      def include?(s)
        s == @name ? true : false
      end
      ...
      def add_note(id, options = {})
        ...
        @notes[id] = RGen::Specs::Note.new(id, options[:type], options)
      end
        ...
        limit_one.members.each do |m|
          if limit_two.respond_to? m
            unless limit_one.send(m) == limit_two.send(m)
              ...
              RGen.log.debug "Found limit difference for member #{m} for #{limit_one} and #{limit_two}"
            end
          else
            # Limit two doesn't have the current instance variable or was not provided
            # as an argument so log a difference
            RGen.log.debug "Member #{m} exists for #{limit_one} and does not for #{limit_two}"
