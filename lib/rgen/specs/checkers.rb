...
def name_audit(name)
  ...
  if name == :inspect
    RGen.log.debug ':inspect is a reserved spec name'
    return nil
  end
  if name.match(/^\d/)
    RGen.log.debug "Spec #{name} starts with a number"
    return nil
  end
  if name.match(/\s+/)
    RGen.log.debug "Spec #{name} contains white space, removing it"
    name.delete!(/\s+/)
  end
...
def limits_ok?
  ...
  if @min.exp.nil? ^ @max.exp.nil?
    ...
    if @typ.exp
      # status = false
      RGen.log.debug "Spec #{@name} has a typical limit defined with either min or max.  They are mutually exclusive, use 'target' when using min or max"
    end
    ...
    if @min.value.is_a?(Numeric) && @max.value.is_a?(Numeric)
      ...
      if @max.value <= @min.value || @min.value >= @max.value
        status = false
        RGen.log.debug "Spec #{@name} has min (#{@min.value}) and max (#{@max.value}) reversed"
      end
      # Check that target is OK
      unless @target.nil?
        if @target.value <= @min.value || @target.value >= @max.value
          status = false
          RGen.log.debug "Spec #{@name} has a target (#{@target.value}) that is not within the min (#{@min.value}) and max #{@max.value}) values"
        end
      end
    end
  end
...
def evaluate_limit(limit)
  ...
  if !!(limit.match(/^\d+\.\d+$/)) || !!(limit.match(/^-\d+\.\d+$/))
    ...
    RGen.log.debug "Found two numbers without an operator in the limit string '#{limit}', choosing the first..."
  ...
  if result == false
    ...
    begin
      ...
      return result.round(4) if result.is_a? Numeric
        ...
        RGen.log.debug "Limit '#{limit}' had to be rescued, storing it as a #{limit.class}"
