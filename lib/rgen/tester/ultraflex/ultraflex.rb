module RGen
  module Tester
    ...
    RGen.deprecate <<-END
      UltraFlex Tester in RGen core has been moved to a dedicated plugin,
      use Testers::UltraFLEX from this plugin instead of RGen::Tester::Ultraflex
    ...
    #
    # == Basic Usage
    #   $tester = RGen::Tester::Ultraflex.new
    ...
    class Ultraflex
      include Tester
      include Parser
      require 'rgen/tester/ultraflex/files'
      include Files

      autoload :Parser, 'rgen/tester/ultraflex/parser'
      autoload :Generator, 'rgen/tester/ultraflex/generator'
      ...
      # supplied directory or in RGen.config.test_program_output_directory
      def parser(prog_dir = RGen.config.test_program_output_directory)
        unless prog_dir
          fail 'You must supply the directory containing the test program sheets, or define it via RGen.config.test_program_output_directory'
        end
