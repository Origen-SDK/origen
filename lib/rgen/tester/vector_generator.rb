  ...
  module Tester
    module VectorGenerator
      ...
      def render(template, options = {})
        ...
        RGen.file_handler.current_file = template
        ...
        RGen.generator.compiler.options[:file] = template
        options.each { |k, v| RGen.generator.compiler.options[k] = v }
        code = RGen.generator.compiler.insert(ERB.new(File.read(template.to_s), 0, RGen.config.erb_trim_mode, eoutvar).result)
        code.strip!
        push_microcode code
      end
      ...
      def _render(method)  # :nodoc:
        if self.respond_to?(method)
          ...
          # Make the file name available to the template
          RGen.generator.compiler.options[:file] = template
          push_microcode RGen.generator.compiler.insert(ERB.new(File.read(template.to_s), 0, RGen.config.erb_trim_mode, eoutvar).result)
        end
      end
      ...
      def ordered_pins(options = {})
        ...
        RGen.profile 'Working out pin pattern order' do
          pinorder = RGen.app.pin_pattern_order.dup
          pinexclude = RGen.app.pin_pattern_exclude.dup

          if RGen.app.pin_pattern_order.last.is_a?(Hash)
            options.merge!(pinorder.pop)
          end
          if RGen.app.pin_pattern_exclude.last.is_a?(Hash)
          ...
          pins = RGen.pin_bank.pins.dup
          pingroups = RGen.pin_bank.pin_groups.dup

          if pinorder && pinorder.size > 0
            pinorder.each do |id|
              # If the ID refers to a pin group
              if group = RGen.pin_bank.pin_groups[id]
