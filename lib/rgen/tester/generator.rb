...
module RGen
  module Tester
    module Generator
      ...
      autoload :IdentityMap,   'rgen/tester/generator/identity_map'
      autoload :FlowControlAPI, 'rgen/tester/generator/flow_control_api'
      ...
      def self.execute_source(file)
        if RGen.tester.doc? && RGen.interface_loaded? && RGen.interface.respond_to?(:filter_source)
          File.open(file) do |f|
            src = f.read
            src = RGen.interface.filter_source(src)
            # With source file wrapping here to ensure that any calls to RGen.app! within
            # the source code will evaluate to the correct app instance
            RGen.with_source_file(file) do
      ...
      def import(file, options = {})
        ...
        file_pipeline << file
        RGen::Tester::Generator.execute_source(file)
        file_pipeline.pop
        base_collection << @collection
      ...
      module ClassMethods
        def new(*args, &block) # :nodoc:
          ...
          RGen.interface.sheet_generators << x
          x
        end
      end
    end
