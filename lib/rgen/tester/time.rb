module RGen
  module Tester
    ...
    # exposed via the 'rgen time' command
    class Time
      require 'yaml'

      TT_LIB_DIR  = "#{RGen.root}/config/test_time/lib"
      TT_FLOW_DIR = "#{RGen.root}/config/test_time/flow"
      ...
      def import_test_flow(file, options = {})
        clear_stats
        @options = options
        if RGen.tester.respond_to?('read_test_times')
          tests = RGen.tester.read_test_times(file, options)
          flow = []
          merge_indexed_tests(tests) do |name, _attrs|
            if import?(name)
              RGen.log.info "imported... #{name}"
              flow << name
            end
          end
          puts ''
      ...
      def populate(name, attrs, options = {})
        ...
        attrs['reference']['target'] = RGen.target.name
        attrs
      end
      ...
      def export_library(lib, options = {})
        ...
        RGen.file_handler.open_for_write(output_library_file(options)) do |f|
          f.puts YAML.dump('tests' => tests)
        end
        puts "Test library exported to: #{RGen.file_handler.relative_path_to(output_library_file(options))}"
      end
      ...
      def export_flow(flow, options = {})
        RGen.file_handler.open_for_write(output_flow_file(options)) do |f|
          f.puts YAML.dump(flow)
        end
        puts "Test flow exported to: #{RGen.file_handler.relative_path_to(output_flow_file(options))}"
      end
      ...
      def calculate_time(flow, library, options = {})
        ...
        unless options[:silent] || options[:summary]
          RGen.log.info 'Test'.ljust(60) + 'Rule'.ljust(40) +
            library.first[1]['reference']['target'].ljust(30) + RGen.target.name
          orig = 0
        end
        forecasted = flow.reduce(0.0) do  |sum, test|
          if library[test]['include'] == false || library[test]['exclude'] == true
            ...
            unless options[:silent] || options[:summary]
              RGen.log.info test.ljust(60) + library[test]['rule'].to_s.ljust(40) +
        ...
        if options[:silent]
          ...
          RGen.log.info RGen.target.name.ljust(50) + "#{forecasted.round(6)}"
        else
          RGen.log.info ''
          RGen.log.info ''.ljust(100) + '---------------'.ljust(30) + '---------------'
          RGen.log.info ''.ljust(100) + "#{orig.round(6)}".ljust(30) + "#{forecasted.round(6)}"
          RGen.log.info ''.ljust(100) + '---------------'.ljust(30) + '==============='
          RGen.log.info ''
        end
      end
