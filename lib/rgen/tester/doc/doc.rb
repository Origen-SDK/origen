module RGen
  module Tester
    ...
    class Doc
      ...
      autoload :Generator, 'rgen/tester/doc/generator'
      autoload :Model, 'rgen/tester/doc/model'
      ...
      def pc(msg)
        ...
        if html_mode
          ...
          RGen.log.info((' ' * @indent) + msg)
        end
      end

      def c1(msg, options = {})
        if generating_program?
          RGen.interface.doc_comments_capture(msg)
        else
      ...
      def pattern_name
        RGen.app.current_job.output_pattern_filename.gsub('.md', '')
      end
      ...
      def pattern_section(msg)
        unless @inhibit_comments
          if generating_program?
            RGen.interface.flow.start_section(name: msg)
            yield
      ...
      def self.generate_program_model(files, options = {})
        ...
        RGen.app.with_doc_tester do
          ...
          RGen.interface.reset_globals
          options[:files] = files
          RGen.file_handler.preserve_and_clear_state do
            RGen.app.runner.launch(options)
          end
        end
        model.target = RGen.target.name
        model
      end
