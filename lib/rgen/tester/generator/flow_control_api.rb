    ...
    module Generator
      module FlowControlAPI
        ...
        def if_ran(test_id, options = {})
          test_id = RGen.interface.filter_id(test_id, options)
        ...
        def find_by_id(id, options = {}) # :nodoc:
          ...
          if options[:search_other_flows]
            RGen.interface.flow_generators.any? do |flow|
        ...
        def track_relationships(test_options = {}) # :nodoc:
          [:id, RELATION_METHODS].flatten.each do |id|
            if test_options[id]
              test_options[id] = RGen.interface.filter_id(test_options[id], test_options)
            end
          end
        ...
        def generate_unique_label(id = nil)
          id = 'label' if !id || id == ''
          label = "#{RGen.interface.app_identifier}_#{id}"
          label.gsub!(' ', '_')
          label.upcase!
          @@labels ||= {}
          @@labels[RGen.tester.class] ||= {}
          @@labels[RGen.tester.class][label] ||= 0
          @@labels[RGen.tester.class][label] += 1
          "#{label}_#{@@labels[RGen.tester.class][label]}"
        end
