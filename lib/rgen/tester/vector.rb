  ...
  module Tester
    ...
    class Vector
      ...
      def update_pin_val(pin)
        vals = pin_vals.split(' ')
        if pin.belongs_to_a_pin_group? && !pin.is_a?(RGen::Pins::PinCollection)
          port = nil
          pin.groups.each { |i| port = i[1] if port.nil? && RGen.tester.ordered_pins.include?(i[1]) } # see if group is included in ordered pins
          if port
            ix = RGen.tester.ordered_pins.index(port) # find index of port
            i = port.index(pin)
          else
            ix = RGen.tester.ordered_pins.index(pin)
            i = 0
          end
        else
          ix = RGen.tester.ordered_pins.index(pin)
        ...
        if RGen.pin_bank.pin_groups.keys.include? pin.id
          val = pin.map { |p| RGen.tester.format_pin_state(p) }.join('')
          vals[ix] = val
        else
          val = RGen.tester.format_pin_state(pin)
          vals[ix][i] = val
        end
      ...
      def ordered_pins
        RGen.app.pin_map.sort_by { |_id, pin| pin.order }.map { |_id, pin| pin }
      end
