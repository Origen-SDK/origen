...
END3
        ...
        if external?(type)
          to = app_users + RGen.app.subscribers_prod + RGen.app.subscribers_dev
          if config.release_email_subject
            subject = "[#{RGen.app.namespace}] New Official Release: #{config.release_email_subject}"
          else
            subject = "[#{RGen.app.namespace}] New Official Release"
          end
        else
          to = admins + RGen.app.subscribers_dev
          if config.release_email_subject
            subject = "[#{RGen.app.namespace}] New Development Tag: #{config.release_email_subject}"
          else
            subject = "[#{RGen.app.namespace}] New Development Tag"
          end
        end
      ...
      def send_regression_complete_notice(stats = RGen.app.runner.stats, options = {})
        stats, options = RGen.app.runner.stats, stats if stats.is_a?(Hash)
        options = {
          to: current_user
        }.merge(options)
...
#{stats.summary_text}
        ...
        subject = "[#{RGen.app.namespace}] Regression - "
        if stats.clean_run?
          subject += 'PASSED'
        else
          subject += 'FAILED'
