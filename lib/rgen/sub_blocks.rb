module RGen
  module SubBlocks
    ...
    def init_sub_blocks(*args)
      ...
      if options
        # Using reg_base_address for storage to avoid class with the original RGen base
    ...
    module Domains
      def domain(name, options = {})
        domains[name] = RGen::Registers::Domain.new(name, options)
      end
      ...
      def domains
        ...
        if @domain_names
          @domain_names.each do |domain|
            if domain.is_a?(RGen::Registers::Domain)
    ...
    # Jumping through some hoops here since many RGen modules talk about an owner,
    # but would prefer to start standardizing on parent in future, so this should give
    # most RGen models a parent method
    module Parent
      def parent
        @owner
      end
    ...
    module Path
      ...
      def path(options = {})
        ...
        if is_a?(RGen::Registers::BitCollection)
    ...
    def owns_registers?
      if regs
        regs.is_a?(RGen::Registers::RegCollection) && !regs.empty?
      else
        false
      end
    end
    ...
    def sub_block(name, options = {})
      ...
      unless klass.respond_to?(:includes_rgen_model)
        puts 'Any class which is to be instantiated as a sub_block must include RGen::Model,'
        puts "add this to #{klass}:"
        puts ''
        puts '  include RGen::Model'
        puts ''
        fail 'Sub block does not include RGen::Model!'
      end
