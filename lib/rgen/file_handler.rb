...
module RGen
  ...
  #
  # An instance of this class is available as RGen.file_handler
  #
  # Some portions of RGen may implement local code to do this, but these
  ...
  class FileHandler
    ...
    def open_list(file)
      ...
      if f
        f = File.open(f, 'r')
      elsif File.exist?("#{RGen.root}/list/#{File.basename(file)}")
        f = File.open("#{RGen.root}/list/#{File.basename(file)}", 'r')
    ...
    def clean_path_to(file, options = {})
      ...
      # Is it a path relative to RGen.root?
      elsif File.exist?("#{RGen.root}/#{file}")
        Pathname.new("#{RGen.root}/#{file}")
      ...
      # Is it a path relative to the current plugin's RGen.root?
      elsif RGen.current_plugin.instance && File.exist?("#{RGen.current_plugin.instance.root}/#{file}")
        Pathname.new("#{RGen.current_plugin.instance.root}/#{file}")
    ...
    def check(path)
      file_plugin = RGen.import_manager.path_within_a_plugin(path)
      if file_plugin
        if RGen.current_plugin.name
          if file_plugin == RGen.current_plugin.name
            return path
          else
            puts "The requested file is from plugin #{file_plugin} and current system plugin is set to plugin #{RGen.current_plugin.name}!"
            fail 'Incorrect plugin error!'
          end
        else
          RGen.current_plugin.temporary = file_plugin
          return path
        end
      else
        return path
    ...
    def all_matches(file, options)
      if RGen.current_plugin.name
        matches = Dir.glob("#{options[:default_dir]}/#{RGen.current_plugin.name}/**/#{file}").sort
    ...
    def clean_path_to_sub_template(file)
      ...
      begin
        ...
        if RGen.lsf.current_command == 'web'
          clean_path_to(file, load_paths: "#{RGen.root}/templates/web")
        else
          clean_path_to(file)
        end
      rescue
    ...
    def inject_import_path(path, options = {})
      ...
      if path =~ /(.*?)\/.*/
        ...
        if import_name == :rgen || import_name == :rgen_core || RGen.import_manager.names.include?(import_name)
          unless import_name == :rgen || import_name == :rgen_core
            root = RGen.app(import_name).root
          end
          if options[:type] == :template
            if import_name == :rgen || import_name == :rgen_core
              path.sub! 'rgen', "#{RGen.top}/templates/shared"
            else
    ...
    def add_extension_to(file)
      ...
      if f.basename('.erb').extname.empty?
        if default_extension
          ...
          "#{f.dirname}/#{f.basename('.erb')}#{Pathname.new(RGen.file_handler.current_file).basename('.erb').extname}.erb"
        end
      else
    ...
    def set_output_directory(options = {})
      ...
      if options[:output]
        ...
        @output_directory = Pathname.new(RGen.config.output_directory)
      end
      if options[:create]
    ...
    def set_reference_directory(options = {})
      ...
      if options[:reference]
        ...
        @reference_directory = Pathname.new(RGen.config.reference_directory)
      end
      if options[:create]
