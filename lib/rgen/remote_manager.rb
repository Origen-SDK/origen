module RGen
  ...
  class RemoteManager
    ...
    def require!
      unless required?
        ...
        while updates_required?
          RGen.log.info 'The following remotes need to be updated, this will now happen automatically:'
          dirty_remotes.each do |name, remote|
            if remote[:path]
              RGen.log.info "  #{name} - #{remote[:path]}"
            else
              RGen.log.info "  #{name} - #{remote[:version]}"
            end
          end
          update!
        end
    ...
    def validate_production_status(force = false)
      if RGen.mode.production? || force
        remotes.each do |_name, remote|
          ...
          version = RGen::VersionString.new(remote[:version])
    ...
    def rgen_root(name)
      rgen_root_for(named_remotes[name])
    end
    ...
    def dir_defined?(remote)
      if remote[:dir].nil?
        RGen.log.error 'A problem was encountered with a configuration remote'
        RGen.log.error "Error: ':dir' must be defined for each remote."
        fail 'Remote error!'
      end
      true
    end

    def rgen_root_for(remote, options = {})
      ...
      if File.exist?("#{root}/config/application.rb")
        ...
        if options[:accept_missing]
          ...
          RGen.log.error 'A problem was encountered with the following remote:'
          RGen.log.error remote
          RGen.log.error 'Please check that all vault, version or path references are correct.'
          RGen.log.error ''
          RGen.log.error 'If you are sure that the remote is setup correctly and this error'
          RGen.log.error 'persists, you can try running the following command to blow away'
          RGen.log.error 'the local remote cache and then try again from scratch:'
          RGen.log.error ''
          RGen.log.error "rm -fr #{ws.remotes_directory}"
          RGen.log.error ''
          fail 'Remote error!'
        end
      end
    ...
    def top_level_remotes
      RGen.app.config.remotes    #+ RGen.app.config.remotes_dev (there are no core remotes at this time)
    end
    ...
    def traverse_remotes(remote, &block)
      ...
      if remote_present?(remote)
        app = RGen.application_instance(rgen_root_for(remote), reload: true)
    ...
    def remote_present?(remote)
      !!rgen_root_for(remote, accept_missing: true)
    end
        ...
        if new[:path]
          if existing[:path]
            if existing[:path] != new[:path]
              RGen.log.error 'Cannot resolve remote dependencies due to conflicting paths:'
              RGen.log.error "  #{name}:"
              RGen.log.error "    - #{existing[:path]}"
              RGen.log.error "    - #{new[:path]}"
              RGen.log.error ''
              fail 'Remote error!'
            end
          else
