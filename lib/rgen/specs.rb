module RGen
  module Specs
    autoload :Spec, 'rgen/specs/spec'
    autoload :Note, 'rgen/specs/note'
    autoload :Exhibit, 'rgen/specs/exhibit'
    autoload :Doc_Resource, 'rgen/specs/doc_resource.rb'
    autoload :Override, 'rgen/specs/override.rb'
    autoload :Power_Supply, 'rgen/specs/power_supply.rb'
    autoload :Mode_Select, 'rgen/specs/mode_select.rb'
    autoload :Version_History, 'rgen/specs/version_history.rb'
    autoload :Creation_Info, 'rgen/specs/creation_info.rb'
      ...
      if specs_to_be_shown.empty?
        ...
        if options[:creating_spec] == false
          ...
          unless [:global, :local, nil].include?(options[:mode])
            ...
            if self == RGen.top_level
              options[:mode] = :global
            else
              options[:mode] = :local
            end
            options[:specs_to_be_shown] = specs_to_be_shown
            RGen.log.debug "re-calling show_specs with options #{options}"
            return show_specs(options)
          end
        end
        RGen.log.debug "Returning no specs for options #{options}"
        return nil
      elsif specs_to_be_shown.size == 1
        print_to_console(specs_to_be_shown) if options[:verbose] == true
        RGen.log.debug "returning one spec #{specs_to_be_shown.first.name}"
        return specs_to_be_shown.first
      else
        RGen.log.debug "returning an array of specs during initial search: #{specs_to_be_shown}"
        print_to_console(specs_to_be_shown) if options[:verbose] == true
      ...
      specs_to_be_shown.each do |spec|
        attrs_to_be_shown.each do |attr_name, attr_struct|
          unless spec.send(attr_name).nil?
            if spec.send(attr_name).class == RGen::Specs::Spec::Limit
