module RGen
  # ****** These notes refer to the pin model for the upcoming RGen V3 ******
  #
  # Pin muxing and grouping on modern SoCs can be incredibly complex, RGen provides the following pin model
  ...
  # *Aliases*
  #
  # Aliases are alternative names/handles to give to pins when using them to create patterns and other IP in RGen.
  ...
  #
  # The RGen pin API will automatically create scoped aliases for functions and package locations as they are
  ...
  # configuration.
  #
  # While aliases are simply pointers to pin objects pin groups will themselves be an RGen object which will be like
  ...
  module Pins
    autoload :Pin,           'rgen/pins/pin'
    autoload :Port,          'rgen/pins/port'
    autoload :PinCollection, 'rgen/pins/pin_collection'
    autoload :PinBank,       'rgen/pins/pin_bank'
    autoload :PinCommon,     'rgen/pins/pin_common'
    autoload :PinClock,      'rgen/pins/pin_clock'
    autoload :PowerPin,      'rgen/pins/power_pin'
    autoload :GroundPin,     'rgen/pins/ground_pin'
    autoload :FunctionProxy, 'rgen/pins/function_proxy'
    ...
    def add_pin(id = nil, options = {}, &_block)
      ...
      if options[:size] && options[:size] > 1
        ...
        if group.size == 1
          RGen.pin_bank.add_pin(group.first, self, options)
        else
          RGen.pin_bank.add_pin_group(group, self, options)
        end
      else
        if power_pin
        ...
        pin.finalize
        RGen.pin_bank.add_pin(pin, self, options)
      end
    end
    ...
    def pin_pattern_order(*pin_ids)
      ...
      pin_ids.each do |id|
        if pin_aliases[id]
          RGen.app.pin_names[pin_aliases[id].first] = id
          id = pin_aliases[id].first
        end
        RGen.app.pin_pattern_order << id
      end
      RGen.app.pin_pattern_order << options unless options.empty?
    end
    ...
    def pin_pattern_exclude(*pin_ids)
      ...
      pin_ids.each do |id|
        ...
        RGen.app.pin_pattern_exclude << id
      end
      RGen.app.pin_pattern_exclude << options unless options.empty?
    end
    ...
    def add_pin_group_alias(new_name, original_name, options = {})
      group = RGen.pin_bank.find_or_create_pin_group(new_name, self, options)
    ...
    def pin_aliases
      ...
      if !(defined? @@pin_aliases_target) ||
         (@@pin_aliases_target != RGen.target.signature)
        RGen::Pins.clear_pin_aliases
        @@pin_aliases_target = RGen.target.signature
      end
      RGen::Pins.pin_aliases
    end
    ...
    def has_power_pin?(id)
      !!RGen.pin_bank.find(id, power_pin: true)
    end
    ...
    def has_ground_pin?(id)
      !!RGen.pin_bank.find(id, ground_pin: true)
    end
    ...
    def add_pin_group(id, *pins, &_block)
      ...
      unless found # not a pin group alias
        group = RGen.pin_bank.find_or_create_pin_group(id, self, options)
        if block_given?
          yield group
        else
          # SMcG:
      ...
      group
      # RGen.pin_bank.add_pin_group(group, self, {:pins_exist => true}.merge(options))
    end
    ...
    def all_pins(id = nil, _options = {}, &_block)
      if id
        pin = RGen.pin_bank.find(id, ignore_context: true)
      else
        RGen.pin_bank.all_pins
      end
    end
    ...
    def all_power_pins(id = nil, _options = {}, &_block)
      if id
        pin = RGen.pin_bank.find(id, ignore_context: true, power_pin: true)
      else
        RGen.pin_bank.all_power_pins
      end
    end
    ...
    def all_ground_pins(id = nil, _options = {}, &_block)
      if id
        pin = RGen.pin_bank.find(id, ignore_context: true, ground_pin: true)
      else
        RGen.pin_bank.all_ground_pins
      end
    end
    ...
    def pin_groups(id = nil, options = {}, &_block)
      id, options = nil, id if id.is_a?(Hash)
      if id
        pin = RGen.pin_bank.find(id, options)
        unless pin
          puts <<-END
...
If you meant to define the pin_group then use the add_pin_group method instead.
        ...
        RGen.pin_bank.pin_groups(options)
      end
    end
    ...
    def power_pin_groups(id = nil, options = {}, &_block)
      id, options = nil, id if id.is_a?(Hash)
      if id
        pin = RGen.pin_bank.find(id, options.merge(power_pin: true))
        unless pin
          puts <<-END
...
If you meant to define the power_pin_group then use the add_power_pin_group method instead.
        ...
        RGen.pin_bank.power_pin_groups(options)
      end
    end
    ...
    def ground_pin_groups(id = nil, options = {}, &_block)
      id, options = nil, id if id.is_a?(Hash)
      if id
        pin = RGen.pin_bank.find(id, options.merge(ground_pin: true))
        unless pin
          puts <<-END
...
If you meant to define the ground_pin_group then use the add_ground_pin_group method instead.
        ...
        RGen.pin_bank.ground_pin_groups(options)
      end
    end
...
If you meant to define the pin then use the add_pin method instead.
        ...
        if options[:power_pin]
          RGen.pin_bank.power_pins
        elsif options[:ground_pin]
          RGen.pin_bank.ground_pins
        else
          RGen.pin_bank.pins
        end
      end
    ...
    def delete_pin(id, options = {})
      ...
      if pin_groups.key? id
        RGen.pin_bank.delete_pingroup(RGen.pin_bank.find_pin_group(id, options))
      elsif pins(id).class.to_s.match(/Pin/)
        RGen.pin_bank.delete_pin(RGen.pin_bank.find(id, options))
      else
