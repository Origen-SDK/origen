...
module RGen
  # Include this module to identify it as a top-level object in RGen, normally
  # an object that represents the SoC/Top-level device.
  #
  # RGen will then fallback on this top-level object to service any register
  ...
  module TopLevel
    ...
    def reset(options = {})
      RGen.app.listeners_for(:before_top_level_reset).each(&:before_top_level_reset)
      if options[:bang]
        RGen.app.listeners_for(:shutdown, top_level: :last).each do |listener|
          listener.shutdown(Pattern.create_options)
        end
        RGen.app.listeners_for(:on_top_level_reset!, top_level: false).each(&:on_top_level_reset!)
      end
      RGen.app.listeners_for(:on_top_level_reset, top_level: false).each(&:on_top_level_reset)
      RGen.app.listeners_for(:reset_registers).each(&:reset_registers)
      if options[:bang]
        RGen.app.listeners_for(:startup).each do |listener|
          listener.startup(Pattern.create_options)
        end
      end
      RGen.app.listeners_for(:after_top_level_reset).each(&:after_top_level_reset)
    end
    ...
    def includes_rgen_top_level?
      true
    end

    # Sets the RGen.top_level.packages array to nil.  Written so packages created in memory can
    ...
    def init_top_level
      RGen.app.add_toplevel_listener(self)
    end
