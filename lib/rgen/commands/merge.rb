...
options = {}
RGen.log.deprecate <<-END

********************************************************************************************
  The 'merge' command has now been deprecated as the same functionality can be achieved by
  performing the following tasks:

  rgen compile [COMPILE DIRECTORY/FILE] -r [DIRECTORY/FILE FOR COMPARISON]

  By using the -r switch, it overrides the default reference directory which rgen uses,
  therefore essentially allowing rgen to diff between your output directory and any directory
...
opt_parser = OptionParser.new do |opts|
  opts.banner = <<-EOT
The reverse of 'rgen compile' to help merge any changes made to compiled files back to the source.
...
* Any differences in files with an ERB source are left to the user to resolve. RGen tries to help by giving you the
  commands to execute to show the differences and the files that must be edited.

Usage: rgen merge [space separated files, lists or directories] [options]
  EOT
