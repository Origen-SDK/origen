...
opt_parser = OptionParser.new do |opts|
  opts.banner = 'Usage: rgen g [space separated patterns or lists] [options]'
...
def self._with_doc_tester(options)
  if options[:doc] || options[:html]
    RGen.app.with_doc_tester(options) do
      yield
    end
  else
    yield
...
_with_doc_tester(options) do
  ...
  RGen.app.load_target!  # This initial load is required to apply any configuration
...
# === Task: $ rgen g [patname/patlist] [options]
...
# convenient:
#
#   rgen g prb_ers_mas_atuf
#   rgen g prb_ers_mas_atuf.rb
#   rgen g nvm_prb_ers_mas_atuf.atp
#   rgen g pattern/erase/prb_ers_mas_atuf.rb
#
# For patterns that use iterators you can supply either the pattern source name or the generated name,
# for example these are equivalent:
#
#   rgen g prb_ers_mas_bx.rb
#   rgen g prb_ers_mas_b0.atp
#
# Multiple patterns can be supplied on the command line and should be comma separated with no spaces
# in between:
#
#   rgen g prb_ers_mas_atuf,prb_ers_mas_bx
#
