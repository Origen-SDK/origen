...
opt_parser = OptionParser.new do |opts|
  opts.banner = <<-STOP
RGen test time profiling and forecasting tools.

Usage: rgen time CMD [args] [options]
...
when 'forecast'
  RGen.target.loop(options) do |options|
    RGen.app.runner.launch(action:   :forecast_test_time,
...
when 'import'
  if !ARGV[1] || !File.exist?(ARGV[1])
    ...
    puts '  rgen time import exec_times/p2_sort1.txt'
    exit 1
  end
  ...
  RGen.target.loop(options) do |options|
    ...
    RGen.app.runner.launch(action:   :import_test_flow,
                           file:     ARGV[1],
                           ref_name: options[:ref_name])

    RGen.app.runner.launch(action:   :import_test_time,
                           file:     ARGV[1],
                           ref_name: options[:ref_name])
  end
...
when 'new'
  if ARGV[1] == 'rules'
    RGen.app.runner.launch(action:            :compile,
                           file:              "#{RGen.top}/templates/time/rules.rb.erb",
                           output:            "#{RGen.root}/config/test_time",
                           quiet:             true,
                           check_for_changes: false
                          )
    # Add this to the environment if not already present...
    env = "#{RGen.root}/config/environment.rb"
    unless File.readlines(env).any? { |l| l =~ /config\/test_time\/rules/ }
      ...
      File.open(env, 'ab') { |f| f.print "require \"\#{RGen.root}/config/test_time/rules\"\n" }
      env = false
    end
    puts ''
    ...
    puts "  dssc ci -new -keep -com \"Initial\" #{RGen.root}/config/test_time/rules.rb"
  elsif ARGV[1] == 'filter'
    RGen.app.runner.launch(action:            :compile,
                           file:              "#{RGen.top}/templates/time/filter.rb.erb",
                           output:            "#{RGen.root}/config/test_time",
                           quiet:             true,
                           check_for_changes: false
                          )
    ...
    unless File.readlines(env).any? { |l| l =~ /config\/test_time\/filter/ }
      File.open(env, 'ab') { |f| f.print "require \"\#{RGen.root}/config/test_time/filter\"\n" }
      env = false
    end
    puts ''
    ...
    puts 'To check this in run:'
    puts "  dssc ci -new -keep -com \"Initial\" #{RGen.root}/config/test_time/filter.rb"

  else
    puts 'A valid identifier for the kind of file to create must be supplied, e.g.'
    puts '  rgen time new rules'
    exit 1
  end
else
  puts "Unknown command, see 'rgen time -h' for a list of commands"
end
