...
opt_parser = OptionParser.new do |opts|
  opts.banner = <<-END
Generate a new RGen application.

This runs the RGen App Generators plugin to generate the new application, this provides many
...
generators regardless of the base RGen version that this command is being launched from.

See the website for more details:

http://rgen.freescale.net/rgen_app_generators

Usage: rgen new [APP_NAME] [options]
END
  opts.on('-d', '--debugger', 'Enable the debugger') {  options[:debugger] = true }
  opts.on('-v', '--version TAG', String, 'Use a specific version of RGen App Generators') { |f| options[:version] = f }
  opts.separator ''
...
version = options[:version] || begin
  plugin = RGen.client.plugin(:rgen_app_generators)
...
if RGen.running_on_windows?
  tmp = 'C:/tmp/rgen_app_generators'
else
  tmp = '/tmp/rgen_app_generators'
end
...
unless all_present
  ...
  File.open("#{tmp}/app_gen#{version}.gem", 'wb') do |f|
    response =  HTTParty.get("http://rgen-hub.am.freescale.net:9292/gems/rgen_app_generators-#{version}.gem")
    if response.success?
...
RGen.with_boot_environment do
  require 'rgen_app_generators'
  RGenAppGenerators.invoke(dir)
end
