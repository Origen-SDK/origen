  ...
  module ModelInitializer
    ...
    module ClassMethods
      ...
      def new(*args, &block) # :nodoc:
        ...
        x = allocate
        x.send(:init_top_level) if x.respond_to?(:includes_rgen_top_level?)
        ...
        is_top_level = x.respond_to?(:includes_rgen_top_level?)
        if x.respond_to?(:wrap_in_controller)
          x = x.wrap_in_controller
        end
        if is_top_level
          RGen.app.listeners_for(:on_top_level_instantiated, top_level: false).each do |listener|
