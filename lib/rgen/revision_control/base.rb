...
module RGen
  module RevisionControl
    ...
    # a remote repository.
    #
    # RGen.app.rc will return an instance of this class for the revision control system used
    # by the current application, the :local attribute will be automatically set to
    # RGen.root and the :remote attribute will be set per the revision control attributes
    ...
    class Base
      ...
      #
      # In the case of an application's revision controller (returned by RGen.app.rc) this method will often
      # return the same directory as RGen.root. However in some cases an application owner may choose to store
      # their application in a sub directory of a larger project entity that is revision controlled. In that
      # case RGen.root will return the sub directory and this method will return the top-level directory
      ...
      #
      # @example
      #   to_local("config/application.rb sync://sync-15088:15088/Projects/common_tester_blocks/rgen/lib")
      #     # => ["/home/r49409/rgen/config/application.rb", "/home/r49409/rgen/lib"]
      def to_local(path)
        local_abs_paths = []
      ...
      # 'v' prefix
      def prefix_tag(tag)
        tag = RGen::VersionString.new(tag)
        if tag.semantic?
          tag.prefixed
        else
          tag
